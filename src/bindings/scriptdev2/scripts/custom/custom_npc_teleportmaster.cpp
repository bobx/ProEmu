...
/* Name     : custom_npc_teleportmaster.cpp
 * Category : NPCs
 * Complete : 100%
 * Author   : Nicolai Dutka @ <http://nicolaidutka.archongames.com/>
 * Modifier : Wortex @ Hellscream Network <http://www.hellscream-wow.com/>
 * Comment  : Teleporter.
 */
...
bool GossipHello_custom_npc_teleportmaster(Player *player, Creature *Creature)
{
	...
	if(!player->getAttackers().empty()){
		player->CLOSE_GOSSIP_MENU();
		Creature->MonsterWhisper("You are in combat!", pPlayer, false);
		return false;
	}
